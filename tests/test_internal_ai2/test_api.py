...
class TestApi(unittest.TestCase):
        ...
        sg_ann = mmda_ann.SpanGroup.from_json(
            {
                ...
                "metadata": {
                    "text": "hello",
                    "id": 999,
                },  # note id not used; it's just in metadata
            }
        )
    ...
    def test_classification_span_group(self) -> None:
        sg_ann = mmda_ann.SpanGroup.from_json(
            {"spans": [{"start": 0, "end": 1}], "metadata": {"text": "hello", "id": 1}}
        )
        ...
        self.assertEqual(sg_api.attributes.dict(), {"label": "label", "score": 0.5})
    ...
    def test_box(self):
        ...
        assert (
            box.to_mmda().coordinates
            == mmdaBox(l=0.1, t=0.1, w=0.1, h=0.1, page=0).coordinates
        )
    ...
    def test_span(self):
        ...
        assert (
            span.to_mmda().to_json()
            == mmdaSpan(
                start=0, end=1, box=mmdaBox(l=0.1, t=0.1, w=0.1, h=0.1, page=0)
            ).to_json()
        )

    def test_box_group(self):
        box_group = mmda_api.BoxGroup(
            boxes=[mmda_api.Box(left=0.1, top=0.1, width=0.1, height=0.1, page=0)],
            id=0,
            type="test",
        ...
        self.assertEqual(mmda_api.BoxGroup.from_mmda(box_group.to_mmda()), box_group)
    ...
    def test_span_group(self):
        ...
        self.assertEqual(mmda_api.SpanGroup.from_mmda(span_group.to_mmda()), span_group)
