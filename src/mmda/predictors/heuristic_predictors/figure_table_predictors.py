...
class FigureTablePredictions(BaseHeuristicPredictor):
    ...
    @staticmethod
    def generate_map_of_layout_to_tokens(vila_dict, layout_parser_overlap, dict_of_pages_layoutparser, key='caption'):
    ...
    def update_vila_caption_dict(self, vila_caption_dict, vila_non_caption_dict):
    ...
    def merge_vila_token_spans(
        ...
        for page, list_of_boxes in vila_caption_dict.items():
            merged_boxes_list[page] = MergeSpans(list_of_spans=list_of_boxes, w=self.w_avg * 1.2,
    ...
    def _predict(self, merged_boxes_caption_dict, merged_boxes_fig_tab_dict, caption_type) -> List[SpanGroup]:
        ...
        """
        predictions = []
        for page in range(len(tqdm(self.doc.images))):
